...
// Assuming you have the following input elements
const rowRangeFrom = document.getElementById('rowRangeFrom'); // Input for 'from' row
const rowRangeTo = document.getElementById('rowRangeTo'); // Input for 'to' row
const operationColumn = document.getElementById('operationColumn'); // Input for operation column
...
function highlightCells() {
    ...
    for (let i = rowFrom - 1; i <= rowTo - 1; i++) {
        if (data[i] && data[i][operationColumnIndex] !== undefined) {
            data[i][operationColumnIndex].style.backgroundColor = "lightyellow"; // Highlight the selected cell
        }
    }
}

// Add event listeners to the row range and operation column inputs
